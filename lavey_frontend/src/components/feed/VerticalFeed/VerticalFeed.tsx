...
export function VerticalFeed({
  ...
  const extendTail = useCallback(() => {
    ...
    setLoopTail((prev) => {
      ...
      return next.length > MAX_LOOP_BUFFER ? next.slice(-MAX_LOOP_BUFFER) : next;
    });
  ...
  const extendHead = useCallback(() => {
    ...
    const count = Math.min(LOOP_CHUNK, profiles.length);
    const startIdx =
      (profiles.length - (loopHead.length % profiles.length) - 1 + profiles.length) %
      profiles.length;
    ...
    setLoopHead((prev) => {
      ...
      return next.length > MAX_LOOP_BUFFER ? next.slice(0, MAX_LOOP_BUFFER) : next;
    });
  ...
  useEffect(() => {
    ...
    return () => el.removeEventListener('scroll', onScroll);
  }, [onNearEndOfFeed, isLocked, profiles.length, fingerprint, displayEntries.length]);
  ...
  }, [extendHead, extendTail, infiniteLoop, isLocked, profiles.length, fingerprint]);
  ...
  return (
    ...
    >
      {infiniteLoop && profiles.length >= 2 ? (
        <div ref={topSentinelRef} className="vertical-feed__sentinel" aria-hidden />
      ) : null}
      ...
      {infiniteLoop && profiles.length >= 2 ? (
        <div ref={bottomSentinelRef} className="vertical-feed__sentinel" aria-hidden />
      ) : null}
    </section>
  );
}
