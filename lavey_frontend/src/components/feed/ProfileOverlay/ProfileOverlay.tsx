...
import { resolveProfileLocationName } from '@/utils/profile/resolveProfileLocationName';
...
export function ProfileOverlay({ profile }: ProfileOverlayProps) {
  ...
  const locationLabel = resolveProfileLocationName(profile);
  ...
  return (
    ...
    >
      {locationLabel ? (
        <div className="profile-overlay__location" aria-label={`Location: ${locationLabel}`}>
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden className="profile-overlay__location-icon">
            <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5A2.5 2.5 0 1 1 12 6a2.5 2.5 0 0 1 0 5.5z" />
          </svg>
          <span className="profile-overlay__location-text">{locationLabel}</span>
        </div>
      ) : null}
      ...
      <div className="profile-overlay__bottom">
        ...
        <div className="profile-overlay__meta">
          ...
          {distanceLabel ? (
            <span className="profile-overlay__meta-dot" aria-hidden>
              ·
            </span>
          ) : null}
