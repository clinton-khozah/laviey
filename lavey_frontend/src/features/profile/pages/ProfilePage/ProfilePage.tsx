...
export function ProfilePage() {
  ...
  return (
    <div className="profile-page">
      <PageScroller>
        ...
        {error && !profile && (
          <FeedState
            message={error}
            onRetry={() =>
              error === SESSION_EXPIRED_MESSAGE ? void signOut() : void refetch()
            }
              ...
              <div className="profile-page__avatar-wrap">
                ...
                {!verified && (
                  ...
                  >
                    ...
                    <span className="profile-page__unverified-label">Unverified</span>
                  </button>
                )}
              </div>
              {avatarUploadError ? (
                <p className="profile-sheet__edit-error profile-page__avatar-error" role="alert">
                  {avatarUploadError}
                </p>
              ) : null}
      ...
      <ProfileSheet open={sheet === 'edit'} title="Edit profile" fromTop hideHandle onClose={closeSheet}>
      ...
      <ProfileSheet open={sheet === 'photos'} title="My photos" fromTop hideHandle onClose={closeSheet}>
        {photoSavePhase === 'success' ? (
          <SheetSaveSuccess action="photo" onComplete={() => setPhotoSavePhase('idle')} />
        ) : (
          <>
        <div className="profile-sheet__photos">
          <p className="profile-sheet__photos-lead">
            Add up to {MAX_PROFILE_POSTS} photos for your profile and discover card. Each photo must
            be 3 MB or less.
          </p>
          ...
          <ProfilePostsGrid
            ...
            addAriaLabel={pendingPhotoFile ? 'Choose a different photo' : 'Choose photo'}
            addButtonLabel={pendingPhotoFile ? 'Change' : 'Add'}
            onAdd={atPostLimit || isUploadingPhoto ? undefined : openPhotoPicker}
          />
          ...
          {isUploadingPhoto ? (
            <div className="profile-sheet__photos-loader" aria-live="polite">
      ...
      <BlockedUsersSheet open={sheet === 'blockedUsers'} onClose={() => setSheet('safety')} />
      ...
      <ProfileSheet open={sheet === 'deleteAccount'} title="Delete account?" fromTop hideHandle onClose={closeSheet}>
        <p style={{ marginBottom: 20, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
          Your profile, matches, messages, and gifts will be permanently deleted. You won&apos;t be
          able to recover this account.
        </p>
        {deleteError && (
          <p style={{ marginBottom: 16, color: 'var(--danger, #e5484d)', fontSize: 14, lineHeight: 1.45 }}>
            {deleteError}
          </p>
        )}
        <button
