...
import { readMeetupChat, writeMeetupChat } from "../utils/meetupChatStorage";
...
function normalizeMessage(message: MeetingChatMessage): MeetingChatMessage {
  return { ...message, likeUserIds: Array.isArray(message.likeUserIds) ? message.likeUserIds : [] };
}

function toggleLikeOnMessage(message: MeetingChatMessage, userId: string, active: boolean): MeetingChatMessage {
...
/**
 * Ephemeral meetup comments — same design as lavey_frontend's useMeetingChat: messages
 * are broadcast live via a Supabase Realtime channel (not persisted server-side) and
 * kept locally per-device so your own history survives a reload.
 */
export function useMeetingChat({ meetupId, localUserId, localDisplayName, localAvatarUrl = "" }: UseMeetingChatOptions) {
  ...
  useEffect(() => {
    let cancelled = false;
    void readMeetupChat(meetupId).then((stored) => {
      if (!cancelled) setMessages(stored.map(normalizeMessage));
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    void writeMeetupChat(meetupId, messages);
  }, [meetupId, messages]);

  useEffect(() => {
    const supabase = getSupabaseRealtimeClient();
    if (!supabase || !meetupId || !localUserId) return;
    ...
    channel
      ...
      .on("broadcast", { event: "chat-like" }, ({ payload }) => {
        ...
        if (!data?.messageId || !data.fromUserId || data.fromUserId === localUserId) return;
        updateMessages((prev) =>
          prev.map((message) => (message.id === data.messageId ? toggleLikeOnMessage(message, data.fromUserId, data.active) : message)),
        );
      })
      .subscribe();
  ...
  return { messages, sendMessage, toggleMessageLike };
}
